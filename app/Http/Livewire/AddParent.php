...
class AddParent extends Component
{
    ...
    public function delete($id){
        ...
        $My_Parent=ParentAttachment::where('parent_id',$id)->pluck('parent_id');
        $Details=My_Parent::where('id',$id)->first();
        if($My_Parent->count()==0){
            ...
            Storage::disk('parent_attachments')->deleteDirectory($Details->National_ID_Father);
            My_Parent::findOrFail($id)->delete();
            flash()->addWarning(trans('messages.Delete'));
            return redirect()->to('/add_parent');
        }
